...
def write_agent_factor_config(agent_root: Path, n_factors: int) -> Path:
    ...
    out.write_text(header + yaml.safe_dump(cfg, sort_keys=False),
                   encoding="utf-8")
    logger.info("wrote %s (n_quantiles=%d for %d factors)", out, n_q, n_factors)
    return out
...
def write_agent_config_snippet(agent_root: Path, signals: List[str]) -> Path:
    ...
    sigs = list(signals)
    text = f"""# Generated by real-data ingest - merge into configs/config.yaml or use as overlay.
data:
  mode: osap
  osap:
    signals: {sigs}
