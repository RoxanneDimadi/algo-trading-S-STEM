...
def mock_bridge(mem_ledger):
    return AlpacaExecutionBridge(
        config=AlpacaConfig(api_key="TEST_KEY_ID",
                            secret_key="TEST_SECRET_KEY", paper=True),
        ledger=mem_ledger,
    )
...
def test_bridge_paper_and_live_url_switching():
    assert AlpacaConfig(paper=True).get_effective_base_url(
    ) == "https://paper-api.alpaca.markets"
    assert AlpacaConfig(paper=False).get_effective_base_url(
    ) == "https://api.alpaca.markets"
    assert (
        AlpacaConfig(
            base_url="https://mock-alpaca.internal:8080/"
...
def test_bridge_extracts_x_request_id_on_success(mock_bridge, mem_ledger):
    ...
    mock_resp.text = json.dumps(
        {"id": "ord_123", "status": "accepted", "symbol": "AAPL",
         "filled_qty": "0"})
    ...
    with patch.object(mock_bridge.session, "request", return_value=mock_resp):
        result = mock_bridge.submit_order(
            TradeDirective(symbol="AAPL", side="buy", qty=15.0))
...
def test_bridge_extracts_x_request_id_on_error(mock_bridge, mem_ledger):
    ...
    with patch.object(mock_bridge.session, "request", return_value=mock_resp):
        result = mock_bridge.submit_order(
            TradeDirective(symbol="MSFT", side="buy", qty=500.0))
...
def test_bridge_handles_network_timeout(mock_bridge, mem_ledger):
    with patch.object(
        ...
        result = mock_bridge.submit_order(
            TradeDirective(symbol="NVDA", side="buy", qty=10.0))
        assert result.success is False
...
def test_evaluator_gp_partial_adjustment_sizing(mock_evaluator, mock_bridge):
    ...
    with patch.object(mock_bridge, "get_account",
        ...
        decision = mock_evaluator.evaluate_signal({
            "ticker": "AAPL", "action": "buy", "price": 200.0,
            "signal_strength": 1.0,
        })
...
def test_evaluator_cost_hurdle_filters_weak_noise(mock_evaluator, mock_bridge):
    with patch.object(mock_bridge, "get_account",
            ...
            patch.object(mock_bridge, "get_position", return_value=None):
        decision = mock_evaluator.evaluate_signal({
            "ticker": "GOOGL", "action": "buy", "price": 150.0,
...
def test_evaluator_short_selling_protection(mock_evaluator, mock_bridge):
    with patch.object(mock_bridge, "get_account",
            ...
            patch.object(mock_bridge, "get_position", return_value=None):
        decision = mock_evaluator.evaluate_signal({
            "ticker": "TSLA", "action": "sell", "price": 250.0,
            "signal_strength": -1.0,
        })
        assert decision.approved is False
...
def test_evaluator_rebalance_with_existing_position(
        mock_evaluator, mock_bridge):
    # hold 30 @ $200 (6%); aim 20%; gamma 0.5 -> target 13% = 65 shares; buy 35
    with patch.object(mock_bridge, "get_account",
                      return_value={"portfolio_value": "100000",
                                    "buying_power": "200000"}), \
            patch.object(mock_bridge, "get_position",
                         return_value={"qty": "30", "current_price": "200.0",
                                       "market_value": "6000.0"}):
        decision = mock_evaluator.evaluate_signal({
            "ticker": "AAPL", "action": "buy", "price": 200.0,
            "signal_strength": 1.0,
...
def test_order_types_validation(mock_bridge):
    res = mock_bridge.submit_order(TradeDirective(
        symbol="AAPL", side="buy", qty=10.0, order_type="limit"))
    assert res.success is False and "limit_price" in str(res.error_message)

    res2 = mock_bridge.submit_order(TradeDirective(
        symbol="AAPL", side="buy", qty=10.0, order_type="stop"))
...
def test_webhook_positions_and_orders_endpoints(test_app, mock_bridge):
    with patch.object(mock_bridge, "get_positions",
                      return_value=[{"symbol": "SPY", "qty": "50"}]):
        assert test_app.get(
            "/positions").get_json()["positions"][0]["symbol"] == "SPY"

    assert test_app.get("/orders").status_code == 200
    assert test_app.get("/decisions").status_code == 200

    with patch.object(mock_bridge, "cancel_all_orders",
                      return_value=(True, "cancel_req_999")):
        resp = test_app.post(
            "/cancel_all", json={"passphrase": "test_secret_passphrase_123"})
        assert resp.status_code == 200
