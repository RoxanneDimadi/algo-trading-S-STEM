...
def test_get_account_success_and_failure(mock_bridge):
    with patch.object(mock_bridge.session, "request", return_value=_ok_resp({"equity": "100000"})):
        account = mock_bridge.get_account()
        assert account["equity"] == "100000"

    bad = _ok_resp({"message": "unauthorized"}, x_request_id="req_fail", status=401)
...
def test_get_positions_list_and_dict_and_error(mock_bridge):
    with patch.object(mock_bridge.session, "request", return_value=_ok_resp([{"symbol": "AAPL"}])):
        assert mock_bridge.get_positions()[0]["symbol"] == "AAPL"

    with patch.object(mock_bridge.session, "request", return_value=_ok_resp({"symbol": "SPY"})):
...
def test_get_position_found_missing_and_error(mock_bridge):
    with patch.object(mock_bridge.session, "request", return_value=_ok_resp({"qty": "10"})):
...
def test_get_clock_success_and_failure(mock_bridge):
    with patch.object(mock_bridge.session, "request", return_value=_ok_resp({"is_open": True})):
...
def test_submit_notional_order_and_invalid_qty(mock_bridge):
    ok = _ok_resp({"id": "n1", "status": "accepted", "filled_qty": "0"}, status=201)
    with patch.object(mock_bridge.session, "request", return_value=ok):
        directive = TradeDirective(symbol="AAPL", side="buy", qty=0.0, notional=1500.0)
...
def test_submit_limit_and_stop_limit_orders(mock_bridge):
    ...
    with patch.object(mock_bridge.session, "request", return_value=ok) as mock_req:
        d = TradeDirective(symbol="MSFT", side="buy", qty=5, order_type="limit", limit_price=400.0)
    ...
    bad_sl = TradeDirective(symbol="MSFT", side="buy", qty=1, order_type="stop_limit", limit_price=100.0)
    assert "stop_price" in (mock_bridge.submit_order(bad_sl).error_message or "")
...
def test_cancel_and_get_orders(mock_bridge):
    ...
    with patch.object(mock_bridge.session, "request", return_value=_ok_resp({"bad": True}, status=500)):
        assert mock_bridge.get_orders() == []

    with patch.object(mock_bridge.session, "request", return_value=_ok_resp([], status=207)):
        success, _ = mock_bridge.cancel_all_orders()
        assert success is True


def test_non_json_response_body_is_audited(mock_bridge, mem_ledger):
    resp = MagicMock()
    ...
    with patch.object(mock_bridge.session, "request", return_value=resp):
        status, data, x_req, err = mock_bridge._request("GET", "/v2/clock")
        assert status == 200
...
def test_evaluator_rejects_missing_symbol_and_bad_action(evaluator):
    ...
    d2 = evaluator.evaluate_signal({"ticker": "AAPL", "action": "explode", "price": 10})
    assert d2.approved is False
    assert "Unknown action" in d2.reason


def test_evaluator_account_fallback_and_nonpositive_equity(evaluator, mock_bridge):
    with patch.object(mock_bridge, "get_account", side_effect=RuntimeError("down")), \
         patch.object(mock_bridge, "get_position", return_value=None):
        d = evaluator.evaluate_signal({"ticker": "AAPL", "action": "buy", "price": 100.0, "signal_strength": 1.0})
        # Fallback portfolio 100k still allows a sized trade
        assert d.approved is True

    with patch.object(mock_bridge, "get_account", return_value={"portfolio_value": "0"}), \
         patch.object(mock_bridge, "get_position", return_value=None):
        d2 = evaluator.evaluate_signal({"ticker": "AAPL", "action": "buy", "price": 100.0})
        assert "non-positive" in d2.reason


def test_evaluator_missing_price_and_position_query_failure(evaluator, mock_bridge):
    with patch.object(mock_bridge, "get_account", return_value={"portfolio_value": "100000", "buying_power": "100000"}), \
         patch.object(mock_bridge, "get_position", side_effect=RuntimeError("pos fail")):
...
def test_evaluator_close_position_and_notional_aim(evaluator, mock_bridge):
    ...
    with patch.object(mock_bridge, "get_account", return_value={"portfolio_value": "100000", "buying_power": "100000"}), \
         patch.object(mock_bridge, "get_position", return_value={"qty": "40", "current_price": "50", "market_value": "2000"}):
        d = evaluator.evaluate_signal({"ticker": "AAPL", "action": "close", "price": 50.0})
        assert d.approved is True
...
def test_evaluator_requested_qty_and_buying_power_downsize(evaluator, mock_bridge):
    with patch.object(mock_bridge, "get_account", return_value={"portfolio_value": "100000", "buying_power": "500"}), \
         patch.object(mock_bridge, "get_position", return_value=None):
...
def test_evaluator_insufficient_buying_power_rejects(evaluator, mock_bridge):
    with patch.object(mock_bridge, "get_account", return_value={"portfolio_value": "100000", "buying_power": "10"}), \
         patch.object(mock_bridge, "get_position", return_value=None):
        d = evaluator.evaluate_signal({
            "ticker": "AAPL", "action": "buy", "price": 100.0,
...
def test_evaluator_max_notional_downsize(evaluator, mock_bridge):
    ...
    with patch.object(mock_bridge, "get_account", return_value={"portfolio_value": "1000000", "buying_power": "1000000"}), \
         patch.object(mock_bridge, "get_position", return_value=None):
        d = evaluator.evaluate_signal({
            "ticker": "AAPL", "action": "buy", "price": 100.0,
...
def test_evaluator_sell_caps_to_available_shares(evaluator, mock_bridge):
    cfg = AgentPolicyConfig(
    ...
    ev = CostAwareAgentEvaluator(bridge=mock_bridge, config=cfg, ledger=mock_bridge.ledger)
    # Hold 10 shares; close aims to 0 => sell 10 (already covered). Here sell toward flat with qty request.
    with patch.object(mock_bridge, "get_account", return_value={"portfolio_value": "100000", "buying_power": "100000"}), \
         patch.object(mock_bridge, "get_position", return_value={"qty": "10", "current_price": "50", "market_value": "500"}):
        d = ev.evaluate_signal({"ticker": "AAPL", "action": "flat", "price": 50.0})
        assert d.approved is True
        assert d.final_directive.side == "sell"
...
def test_ledger_jsonl_and_order_status_update(tmp_path):
    ...
    ledger.record_api_call(ApiAuditEntry(method="GET", endpoint="/v2/clock", status_code=200, success=True))
    ledger.record_agent_decision(AgentDecisionEntry(symbol="AAPL", approved=False, reason="SKIPPED: dust"))
    ledger.record_order(OrderLedgerEntry(client_order_id="c1", symbol="AAPL", side="buy", qty=1.0, status="new"))
    ledger.update_order_status("c1", status="filled", filled_qty=1.0, filled_avg_price=10.5)
...
def test_webhook_header_auth_and_invalid_json(test_app, mock_bridge):
    with patch.object(mock_bridge, "get_account", return_value={"portfolio_value": "100000", "buying_power": "200000"}), \
         patch.object(mock_bridge, "get_position", return_value=None), \
         patch.object(mock_bridge.session, "request", return_value=_ok_resp({"id": "o1", "status": "accepted", "filled_qty": "0"}, status=201)):
        resp = test_app.post(
            "/api/v1/webhook",
            data=json.dumps({"ticker": "AAPL", "action": "buy", "price": 100.0, "signal_strength": 1.0}),
            headers={"Content-Type": "application/json", "X-Webhook-Secret": "secret"},
        )
        assert resp.status_code == 200
        assert resp.get_json()["status"] == "success"

    bad = test_app.post("/webhook", data="{{{not json", headers={"Content-Type": "application/json", "X-Webhook-Secret": "secret"})
...
def test_webhook_rejected_vs_skipped_status_codes(test_app, mock_bridge):
    with patch.object(mock_bridge, "get_account", return_value={"portfolio_value": "100000", "buying_power": "200000"}), \
        ...
        r1 = test_app.post("/webhook", json={"passphrase": "secret", "ticker": "AAPL", "action": "nope", "price": 10})
        assert r1.status_code == 422
        assert r1.get_json()["decision"] == "rejected"

        # Skipped (weak signal under hurdle) — raise hurdle via temporary evaluator config already at 0;
        # use dust: strength tiny with high price / tiny quantity won't skip under gamma=1 with 0 hurdle.
        # Force skip by dust notional via existing tiny delta: hold at target already.
        with patch.object(mock_bridge, "get_position", return_value={"qty": "200", "current_price": "100", "market_value": "20000"}):
...
def test_webhook_account_endpoint_and_cancel_unauthorized(test_app, mock_bridge):
    with patch.object(mock_bridge, "get_account", return_value={"equity": "1"}):
        r = test_app.get("/account")
        assert r.status_code == 200
        assert r.get_json()["status"] == "success"

    with patch.object(mock_bridge, "get_account", side_effect=RuntimeError("nope")):
        r2 = test_app.get("/account")
        assert r2.status_code == 500

    with patch.object(mock_bridge, "get_positions", side_effect=RuntimeError("nope")):
...
def test_public_mode_webhook_without_passphrase(mock_bridge, evaluator, mem_ledger):
    app = create_webhook_app(
        bridge=mock_bridge,
        evaluator=evaluator,
    ...
    with patch.object(mock_bridge, "get_account", return_value={"portfolio_value": "100000", "buying_power": "200000"}), \
         patch.object(mock_bridge, "get_position", return_value=None), \
         patch.object(mock_bridge.session, "request", return_value=_ok_resp({"id": "p1", "status": "accepted", "filled_qty": "0"}, status=201)):
        resp = client.post("/webhook", json={"ticker": "AAPL", "action": "buy", "price": 100.0, "signal_strength": 1.0})
        assert resp.status_code == 200


def test_health_reports_alpaca_error_status(mock_bridge, evaluator, mem_ledger):
    app = create_webhook_app(bridge=mock_bridge, evaluator=evaluator, webhook_passphrase="secret", ledger=mem_ledger)
    client = app.test_client()
    bad = _ok_resp({"message": "auth failed"}, status=401, x_request_id="h_err")
    with patch.object(mock_bridge.session, "request", return_value=bad):
